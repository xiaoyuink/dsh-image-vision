...
- 支持 `png` / `jpeg` / `webp` / `gif`，单张 ≤ 20MB。
- 图片保存到草稿目录（插件目录本体内）`~/.dsh/plugin/dsh-image-vision/drafts/`，并在输入框插入图片引用标记（如 `![图片](i/iv-xxx.png)`）。
- 发送后，模型按注入的系统规则自动调用 `image_vision` 识别并回复。
- 无论当前模型是否识图都可用：识图则主模型直接分析，不识图则走插件视觉模型。
- 草稿图片为临时文件，**保存 20 分钟后自动清理**。
...
- 仅当插件配置了视觉模型（且总开关开启）时才注册；未配置时选择器保持干净，不出现 👁 模型。
...
- **总开关**：开启后在模型选择器显示 👁 视觉组 + 输入框显示视觉模型选择器；关闭后插件完全不参与（视觉组、工具、系统提示、输入框按钮全部注销/不渲染）。
...
| 方法 | 路径 | 用途 |
| --- | --- | --- |
| GET | `/i/<name>` | 草稿图片短路由（输入框预览/消息渲染） |
| GET | `/api/dsh-image-vision/draft-image?name=` | 草稿图片服务 |
| POST | `/api/dsh-image-vision/upload` | 输入框图片上传落盘 |
