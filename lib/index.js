...
// 版本号优先从 package.json 动态读取（发布时只需改 package.json，避免两处失同步）；
// 读取失败才回退到内置常量。
let PLUGIN_VERSION = "2.7.2";
try {
  const _pkgRaw = readFileSync(join(dirname(fileURLToPath(import.meta.url)), "..", "package.json"), "utf8");
  const _pkg = JSON.parse(_pkgRaw);
  if (_pkg && typeof _pkg.version === "string" && _pkg.version !== "") PLUGIN_VERSION = _pkg.version;
} catch { /* 保留内置常量 */ }
...
const SettingsSchema = z.object({
  ...
  // 遇到主模型/插件模型无法直接读取的格式（AI/EPS 等矢量图）需用 Python 转换时，
  // 是否允许插件自动安装缺失依赖（PyMuPDF / Ghostscript）。默认不安装，需用户明确同意。
  allowAutoInstallPythonDeps: z.boolean().default(false).description("遇到 AI/EPS 等矢量图需要 Python 转换时，允许自动安装缺失依赖（PyMuPDF / Ghostscript）"),
});
...
async function saveAsAttachment(ctx, path, signal) {
  ...
  const { bytes: out, mediaType } = await normalizeImageBytes(resolved, bytes);
...
function dataUrlFor(path, bytes, mediaType) {
  const mime = mediaType ?? mediaTypeOf(path) ?? "image/png";
  return `data:${mime};base64,${toBase64(bytes)}`;
}

/** 可直接交给视觉模型/attachment 的网络安全格式。 */
const WEB_SAFE_IMAGE_TYPES = new Set(["image/png", "image/jpeg", "image/webp", "image/gif"]);

/**
 * 把图片字节规范化为视觉模型/attachment 可直接使用的格式：
 * - png/jpeg/webp/gif：原样返回；
 * - TIFF/SVG/其他：用 sharp 光栅化为 PNG；
 * - Adobe AI/EPS/PS 等专有矢量：sharp 无法解析，改用 Python（PyMuPDF + Ghostscript）转换；
 * - 都无法处理：给出明确提示。
 * 返回 { bytes, mediaType }。
 */
async function normalizeImageBytesFor(mediaType, bytes, path) {
  if (mediaType !== void 0 && WEB_SAFE_IMAGE_TYPES.has(mediaType)) {
    return { bytes, mediaType };
  }
  try {
    const sharp = await loadSharp();
    const out = await sharp(Buffer.from(bytes), { failOn: "none" }).rotate().png().toBuffer();
    return { bytes: out, mediaType: "image/png" };
  } catch (sharpErr) {
    const ext = String(path ?? "").toLowerCase();
    const isVectorAiEps = /\.(ai|eps|ps)$/.test(ext) || mediaType === "application/illustrator" || mediaType === "application/postscript";
    if (isVectorAiEps) {
      const result = await convertVectorWithPython(bytes, path, mediaType);
      if (result.ok) return { bytes: result.bytes, mediaType: "image/png" };
      throw new Error(vectorConversionMessage(result, path ?? mediaType ?? "该图片"));
    }
    throw new Error(`图片格式转换失败（${path ?? mediaType ?? "未知"}）: ${String(sharpErr?.message ?? sharpErr)}`);
  }
}

/** 按路径后缀推断格式后规范化（供读取本地文件路径的工具用）。 */
async function normalizeImageBytes(path, bytes) {
  return normalizeImageBytesFor(mediaTypeOf(path), bytes, path);
}

/** 从 PATH 中查找可执行文件（Windows 上也检查 .exe/.cmd/.bat）。 */
...
/** 解析用于矢量图转换的 Python 解释器：环境变量 → PATH 上的 python/python3/py；找不到返回 null。 */
function resolvePythonPath() {
  if (process.env.DSH_IMAGE_VISION_PYTHON) return process.env.DSH_IMAGE_VISION_PYTHON;
  for (const name of ["python", "python3", "py"]) {
    const p = whichExe(name);
    if (p) return p;
  }
  return null;
}

/** 用户是否已同意自动安装 Python 依赖（由 applyInner 注入读取设置，默认不同意）。 */
let readPythonAutoInstall = () => false;

/** 若 python 位于 conda 环境，返回环境名（如 claude-env），否则 null。 */
function condaEnvNameFromPython(pythonPath) {
  const m = /[\\/]envs[\\/]([^\\/]+)[\\/]/.exec(String(pythonPath ?? ""));
  return m ? m[1] : null;
}

/** 运行 Python 脚本（stdio ignore；以退出码 0 判定成功）。resource: 脚本路径或 -m 等参数。 */
function runPython(pythonPath, resource, args) {
  return new Promise((resolve, reject) => {
    const child = spawn(pythonPath, [resource, ...args], { stdio: "ignore" });
    child.once("error", reject);
    child.once("exit", (code, signal) => {
      if (code === 0) resolve();
      else reject(new Error(signal === null ? `python 退出码 ${code}` : `python 被终止: ${signal}`));
    });
  });
}

/** 检查 python 是否具备转换依赖：返回 { fitz, gs }。 */
async function checkPythonDeps(pythonPath, scriptPath) {
  const out = { fitz: false, gs: false };
  try {
    const code = await new Promise((resolve) => {
      const child = spawn(pythonPath, [scriptPath, "--check"], { stdio: "ignore" });
      child.once("error", () => resolve(-1));
      child.once("exit", (c) => resolve(c ?? -1));
    });
    if (code === 0) { out.fitz = true; out.gs = true; }
    else if (code === 1) { out.fitz = false; out.gs = true; }
    else if (code === 2) { out.fitz = true; out.gs = false; }
    // code 3 / -1：两者（或运行）失败，均保持 false
  } catch { /* 保留 false */ }
  return out;
}

/** 尝试自动安装缺失依赖：PyMuPDF 用 pip；Ghostscript 用 conda（若 python 在 conda 环境中）。 */
async function installPythonDeps(pythonPath) {
  const out = { fitz: false, gs: false };
  try {
    await runPython(pythonPath, "-m", ["pip", "install", "--quiet", "PyMuPDF"]);
    out.fitz = true;
  } catch { out.fitz = false; }
  const conda = whichExe("conda");
  if (conda) {
    try {
      const envName = condaEnvNameFromPython(pythonPath);
      const args = envName
        ? ["install", "-n", envName, "-y", "-c", "conda-forge", "ghostscript"]
        : ["install", "-y", "-c", "conda-forge", "ghostscript"];
      await runPython(conda, "install", args.slice(1));
      out.gs = true;
    } catch { out.gs = false; }
  } else {
    out.gs = false;
...
/** 生成转换失败时对使用者的中文提示（区分 无 Python / 缺依赖未同意 / 装依赖失败 / 转换失败）。 */
function vectorConversionMessage(result, label) {
  const name = label || "该图片";
  if (result.reason === "no_python") {
    return `${name} 是 AI/EPS 矢量图，需要 Python 环境转换为 PNG 后才能识别。你的电脑上未找到 Python：请安装 Python 环境，或手动把图片导出为 PNG/JPEG 后再发送。`;
  }
  if (result.reason === "deps_missing") {
    const parts = [];
    if (result.missing && !result.missing.fitz) parts.push("PyMuPDF");
    if (result.missing && !result.missing.gs) parts.push("Ghostscript");
    return `${name} 是 AI/EPS 矢量图，转换需要 Python 依赖（${parts.join("、") || "PyMuPDF/Ghostscript"}）。你可在插件设置页勾选「允许自动安装 Python 依赖」后重试，或手动安装：python -m pip install PyMuPDF（Ghostscript 请从官网安装）。也可手动把图片导出为 PNG/JPEG 后再发送。`;
  }
  if (result.reason === "deps_install_failed") {
    const parts = [];
    if (result.missing && !result.missing.fitz) parts.push("PyMuPDF");
    if (result.missing && !result.missing.gs) parts.push("Ghostscript（请从 https://ghostscript.com 安装，或用 conda 安装）");
    return `${name} 自动安装依赖未完全成功（缺少：${parts.join("、")}）。请按提示手动安装，或手动把图片导出为 PNG/JPEG 后再发送。`;
  }
  if (result.reason === "convert_failed") {
    return `${name} 转换失败：${String(result.detail ?? "未知错误")}。请尝试手动把图片导出为 PNG/JPEG 后再发送。`;
  }
  return `${name} 转换失败（未知原因）。请手动把图片导出为 PNG/JPEG 后再发送。`;
}

/**
 * 用 Python（PyMuPDF + Ghostscript）把 AI/EPS/PS/PDF 矢量图转换为 PNG 字节。
 * 流程：无 Python → {reason:"no_python"}；缺依赖且未同意自动安装 → {reason:"deps_missing"}；
 * 缺依赖但已同意 → 尝试安装后重试；安装失败 → {reason:"deps_install_failed"}；
 * 成功 → { ok:true, bytes }。
 */
async function convertVectorWithPython(bytes, path, mediaType) {
  const pythonPath = resolvePythonPath();
  if (!pythonPath) return { ok: false, reason: "no_python" };
  const scriptPath = join(dirname(fileURLToPath(import.meta.url)), "..", "assets", "convert_vector.py");
  if (!existsSync(scriptPath)) {
    return { ok: false, reason: "convert_failed", detail: "插件自带的 convert_vector.py 缺失" };
  }
  let deps = await checkPythonDeps(pythonPath, scriptPath);
  if (!deps.fitz || !deps.gs) {
    if (readPythonAutoInstall()) {
      await installPythonDeps(pythonPath);
      deps = await checkPythonDeps(pythonPath, scriptPath);
      if (!deps.fitz || !deps.gs) {
        return { ok: false, reason: "deps_install_failed", missing: deps };
      }
    } else {
      return { ok: false, reason: "deps_missing", missing: deps };
    }
  }
  const lower = String(path ?? "").toLowerCase();
  let inExt;
  if (lower.endsWith(".eps")) inExt = ".eps";
  else if (lower.endsWith(".ps")) inExt = ".ps";
  else if (lower.endsWith(".ai")) inExt = ".ai";
  else if (mediaType === "application/postscript") inExt = ".eps";
  else inExt = ".ai";
  const token = `${Date.now()}-${Math.random().toString(36).slice(2)}`;
  const inPath = join(tmpdir(), `iv-vector-${token}${inExt}`);
  const outPath = join(tmpdir(), `iv-vector-${token}.png`);
  try {
    await writeFile(inPath, Buffer.from(bytes));
    await runPython(pythonPath, scriptPath, [inPath, outPath]);
    return { ok: true, bytes: await readFile(outPath) };
  } catch (err) {
    return { ok: false, reason: "convert_failed", detail: String(err?.message ?? err) };
  } finally {
    try { await unlink(inPath); } catch { /* 忽略 */ }
    try { await unlink(outPath); } catch { /* 忽略 */ }
  }
}
...
async function callVisionModel(ctx, provider, modelId, imagePaths, prompt, signal) {
  ...
  for (const p of imagePaths) {
    ...
    const norm = await normalizeImageBytes(path, bytes);
    images.push({ path, bytes: norm.bytes, mediaType: norm.mediaType });
  }
...
async function callAnthropic(baseUrl, apiKey, model, images, prompt, signal) {
  ...
  for (const img of images) {
    ...
    const mime = img.mediaType ?? mediaTypeOf(img.path) ?? "image/png";
    content.push({
      type: "image",
...
async function callOpenAI(baseUrl, apiKey, model, images, prompt, signal) {
  ...
  for (const img of images) {
    content.push({
      type: "image_url",
      image_url: { url: dataUrlFor(img.path, img.bytes, img.mediaType) },
    });
  }
  content.push({ type: "text", text: prompt });
...
function normalizeConfig(raw) {
  ...
  return { providers, active, enabled: src.enabled === true, allowAutoInstallPythonDeps: src.allowAutoInstallPythonDeps === true };
}
    ...
    handler: async (req, res) => {
      try {
        if (req.method === "GET") {
          ...
          sendJson(res, 200, { config: cfg, version: PLUGIN_VERSION });
          return;
        }
        if (req.method === "POST") {
          const body = await readJsonBody(req);
          ...
          await settings.update(SETTINGS_NS, {
            ...
            allowAutoInstallPythonDeps: merged.allowAutoInstallPythonDeps,
  ...
  return ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        // 预处理：非 web-safe（tiff/svg/ai/eps 等）用 sharp 转成 PNG，保证能交给视觉模型/附件存储。
        const name = typeof body.name === "string" && body.name !== "" ? body.name : void 0;
        let norm;
        if (WEB_SAFE_IMAGE_TYPES.has(mediaType)) {
          if (!verifyImageMagic(bytes, mediaType)) {
            sendJson(res, 400, { error: "图片文件头校验失败：文件内容与声明的类型不符" });
            return;
          }
          norm = { bytes, mediaType };
        } else {
          try {
            norm = await normalizeImageBytesFor(mediaType, bytes, name);
          } catch (err) {
...
/**
 * 运行官方文档记载的安装命令，从 GitHub Release 的预构建 tarball 安装：
 *   dsh plugin --profile <profile> add <tarball url>
...
function installUpdate(version) {
  ...
  // Release 资产命名：xiaoyuink-dsh-image-vision-<version>.tgz（见仓库 Release 流程）。
  const tarballUrl = `https://github.com/xiaoyuink/dsh-image-vision/releases/download/v${version}/xiaoyuink-dsh-image-vision-${version}.tgz`;
  const command = process.platform === "win32" ? "dsh.cmd" : "dsh";
  const child = spawn(command, ["plugin", "--profile", profile, "add", tarballUrl], {
...
/** 预设提示词查询路由：GET /api/dsh-image-vision/preset?name=histopathology
 * 返回指定预设的提示词文本；name 缺省或非法时返回 general。
 * 供客户端在「视觉主模型发图」时，按发送文本中的预设关键词自动拼进对应提示词。 */
function registerPresetRoute(ctx) {
  return ctx.webServer.register({
    kind: "exact",
    path: "/api/dsh-image-vision/preset",
    handler: async (req, res) => {
      if (req.method !== "GET") {
        sendJson(res, 405, { error: "method not allowed" });
        return;
      ...
      try {
        const url = new URL(req.url ?? "/", "http://x");
        const name = url.searchParams.get("name") ?? "";
        const preset = name in PRESET_PROMPTS ? name : "general";
        sendJson(res, 200, { name: preset, prompt: PRESET_PROMPTS[preset] });
      } catch {
        sendJson(res, 500, { error: "读取预设失败" });
      }
    },
  });
}
...
function applyInner(ctx) {
  ...
  ctx.inject(["settings"], (settingsCtx) => {
    ...
    readPythonAutoInstall = () => Boolean(scope.get().allowAutoInstallPythonDeps);
