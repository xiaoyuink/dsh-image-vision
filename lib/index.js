...
// 自动识图 twin 路由：为每个文本 provider 注册 "<provider>-vision"，声明图片输入，
// 让 DSH 原生图片草稿流程（粘贴/上传/预览/消息区渲染）通过发送准入。
...
/** 递归改写消息 content：image block → 文本标记（含 tool-result 嵌套）。返回是否改写。 */
async function rewriteImagesDeep(ctx, blocks, signal) {
  if (!Array.isArray(blocks)) return { changed: false, content: blocks };
  let changed = false;
  const content = [];
  for (const block of blocks) {
    if (block && block.type === "image" && block.attachment) {
      content.push({ type: "text", text: await imageBlockToMarker(ctx, block.attachment, signal) });
      changed = true;
    } else if (block && block.type === "tool-result" && Array.isArray(block.content)) {
      const inner = await rewriteImagesDeep(ctx, block.content, signal);
      if (inner.changed) {
        content.push({ ...block, content: inner.content });
        changed = true;
      } else {
        content.push(block);
      }
...
/**
 * 生成一个 provider 的「+ 自动识图」twin adapter：
 * - listModels/resolveModel 镜像原 provider 的模型并声明 inputModalities 含 image → 发送准入通过；
 * - stream 把 image blocks 落盘改写为 `![图片](i/xxx)` 标记后，委托原 provider 的 adapter
 *   （DeepSeek 只见文本；会话日志保留 image block，UI 照常显示图片）。
 */
function makeVisionTwinAdapter(ctx, provider, displayName) {
  const route = `${provider}${VISION_TWIN_SUFFIX}`;
  const original = () => {
    try {
  ...
  return {
    providerInfo(p) {
      // 厂商名 + 「+ 视觉」，如 DeepSeek + 视觉
      return { id: p, name: `${displayName} + 视觉` };
    },
    providerRetryPolicy() {
      try {
        return ctx.llm.registration(provider).retryPolicy;
      } catch {
        return void 0;
      }
    },
    async listModels() {
      ...
      try {
        const listed = await real.listModels(provider);
        return listed.map((m) => ({
          ...m,
          // 视觉版条目名：原模型名 + 小眼睛图标，如 deepseek-v4-flash👁
          name: `${m.name}👁`,
          provider: route,
    ...
    async resolveModel(_p, model, signal) {
      const real = original();
      if (real === void 0) throw new Error(`dsh-image-vision: 文本 provider "${provider}" 的 adapter 不可用`);
      const base = await real.resolveModel(provider, model, signal);
    ...
    async *stream(options) {
      ...
      for (const message of messages) {
        if (message && Array.isArray(message.content)) {
          const r = await rewriteImagesDeep(ctx, message.content, options.signal);
          next.push(r.changed ? { ...message, content: r.content } : message);
        } else {
          next.push(message);
        }
      }
      yield* ctx.llm.stream({ ...options, provider, messages: next });
    },
  };
}

/**
 * 同步「+ 自动识图」twin 路由：为 DSH 装配的每个文本厂商都注册一个视觉组
 * （「X + 视觉」，排在对应厂商组后面），跟随 llm provider 注册表变化热增/热减。
 * 仅当插件配置了视觉模型（hasVision=true）时才注册；未配置时注销全部视觉组，
 * 模型选择器保持干净（不出现带 👁 的模型）。
 * twin 声明 image 输入 → 原生图片附件通过 DSH 发送准入（选「X + 视觉」组后，粘贴图片
 * 可预览、发送、点开预览）；stream 把图片 blocks 落盘改写为 `![图片](i/xxx)` 标记后
 * 委托原文本模型，模型按系统规则调用 image_vision 工具识别。
 */
function syncVisionTwins(ctx, twinHandles, hasVision) {
  // 未配置视觉模型：注销全部已注册 twin，选择器不显示视觉组
  if (!hasVision) {
    for (const [p, handle] of twinHandles) {
      try { handle(); } catch { /* 注销失败忽略 */ }
    }
    twinHandles.clear();
    return;
  }
  let providers = [];
  try {
    providers = ctx.llm.listProviders().map((p) => p?.id).filter((x) => typeof x === "string" && x !== "");
  } catch {
    return;
  }
  const wanted = providers.filter(
    (p) => p !== "dsh-image-vision" && !p.endsWith(VISION_TWIN_SUFFIX),
  );
  // 注销已消失的 provider 的 twin
  for (const [p, handle] of twinHandles) {
    if (!wanted.includes(p)) {
      try { handle(); } catch { /* 注销失败忽略 */ }
      twinHandles.delete(p);
    }
  }
  // 注册新增的 provider 的 twin
  for (const p of wanted) {
    if (twinHandles.has(p)) continue;
    try {
      const info = ctx.llm.listProviders().find((x) => x?.id === p);
      const name = info?.name ?? p;
      const handle = ctx.llm.registerAdapter([`${p}${VISION_TWIN_SUFFIX}`], makeVisionTwinAdapter(ctx, p, name));
      twinHandles.set(p, handle);
    } catch { /* 注册冲突等跳过 */ }
  }
}
...
const SettingsSchema = z.object({
  ...
  visionGroup: z.boolean().default(false).description("模型选择器中显示「+ 自动识图」视觉组（带 👁 的模型条目，需总开关开启）。默认关闭：不注册视觉组、模型选择器保持简洁；此时图片发送由发送层 hook 改写为 `![图片](i/iv-xxx.jpg)` 草稿引用，文本模型同样可用，像素精读链路（ground/crop/ocr）不受影响"),
...
async function currentModelHasVision(ctx, signal) {
  ...
  // 「+ 自动识图」twin 路由声明了 image 输入（用于通过 DSH 发送准入），
  // 但实际底层是文本模型：对 image_vision 而言必须按"不识图"处理，走外部视觉模型。
  if (String(sel.provider).endsWith(VISION_TWIN_SUFFIX)) return false;
  try {
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const result = await testVisionModel(
          ...
          fallbackApiKey(getConfig, String(body.apiBaseUrl ?? ""), String(body.apiKey ?? "")),
          String(body.model ?? ""),
        );
...
/** 解析供应商 API Key：支持 `env:VAR_NAME` 引用（从进程环境变量读取，配置文件不落明文）。 */
function resolveApiKey(provider) {
  const raw = String(provider?.apiKey ?? "");
...
/** 测试/发现/余额路由的 Key 回落：提交值脱敏或为空时，从当前配置按 baseUrl 匹配供应商取真实 Key。 */
function fallbackApiKey(getConfig, baseUrl, submitted) {
  if (!isMaskedOrEmpty(submitted)) return submitted;
  try {
    const cfg = normalizeConfig(getConfig());
    const b = String(baseUrl ?? "").replace(/\/+$/, "");
    const p = cfg.providers.find((x) => String(x.apiBaseUrl ?? "").replace(/\/+$/, "") === b) ?? cfg.providers[0];
    return p ? resolveApiKey(p) : "";
  } catch {
    return "";
  }
}
  ...
  return ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const apiKey = fallbackApiKey(getConfig, baseUrl, String(body.apiKey ?? ""));
        if (!baseUrl) {
...
function registerConfigRoutes(ctx, getConfig, onSaved) {
  return ctx.webServer.register({
    ...
    handler: async (req, res) => {
      try {
        if (req.method === "GET") {
          // 回显脱敏：明文 Key 不返回浏览器（env: 引用不是密钥本身，原样显示便于查看）
          const cfg = normalizeConfig(getConfig());
          for (const p of cfg.providers) {
            if (p.apiKey !== "" && !p.apiKey.startsWith("env:")) p.apiKey = MASKED_KEY;
          }
        ...
        if (req.method === "POST") {
          ...
          // 脱敏占位 → 保留原 Key（设置页"留空 = 不修改"）；env: 引用原样保留
...
function applyInner(ctx) {
  ...
  ctx.inject(["settings"], (settingsCtx) => {
    ...
    // settings 就绪后重算工具/视觉组/系统提示（延迟到 apply 完成后再执行，
    // 避免同步回调阶段引用尚未初始化的 toolDisposers 等容器）。
    queueMicrotask(refreshState);
  });

  // 「+ 自动识图」twin 路由（视觉组）：默认关闭；仅当配置开启 visionGroup 时才注册
  // （声明图片输入 → 原生图片附件的预览/发送/点开大图全流程可用）。关闭时模型选择器
  // 不出现带 👁 的模型项，图片发送由 client 发送层 hook 改写为 `![图片](i/iv-xxx.jpg)`
  // 草稿引用（纯文本发送，文本模型同样可用；像素精读链路 ground/crop/ocr 不受影响）。
  const twinHandles = new Map();
  // 状态刷新：总开关开启时注册识图工具 + 系统提示；关闭时全部注销（等效插件不存在）。
  // 视觉组是否注册仅取决于 visionGroup 开关（且需总开关开启、已配置视觉模型）。
  function refreshState() {
    const cfg = configSource();
    const on = configHasVisionModel(cfg);
    const wantsTwins = on && cfg.visionGroup === true;
    try { syncVisionTwins(ctx, twinHandles, wantsTwins); } catch { /* 同步失败忽略 */ }
    if (on) {
  ...
  ctx.effect(() => {
    const disposers = [
      ...
      // onSaved：配置保存/激活后立即重同步视觉组（添加/删除视觉模型即时生效）。
  ...
  // 视觉组 + 识图工具 + 系统提示 生命周期：跟随 llm provider 注册表变化 + 插件配置变化（refreshState）
  ctx.effect(() => {
    refreshState();
    ...
    return () => {
      ...
      for (const handle of twinHandles.values()) {
        try { handle(); } catch { /* 注销失败忽略 */ }
      }
      twinHandles.clear();
    };
