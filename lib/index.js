...
const inject = ["tools", "llm", "agentDefaultModel", "attachments", "fs", "webServer", "systemPrompt"];

const SETTINGS_NS = settingsNamespace("image-vision");

// 输入框粘贴/上传的图片草稿落盘目录（~/.dsh/image-vision/drafts/）。
const DRAFTS_DIR = join(homedir(), ".dsh", "image-vision", "drafts");
...
function makeVisionTwinAdapter(ctx, provider, displayName) {
  ...
  return {
    providerInfo(p) {
      return { id: p, name: `${displayName} + 自动识图` };
    },
    providerRetryPolicy() {
      try {
    ...
    async listModels() {
      ...
      try {
        ...
        return listed.map((m) => ({ ...m, provider: route, inputModalities: ["text", "image"] }));
      } catch {
        return [];
      }
    },
...
/** 同步「+ 自动识图」twin 路由：跟随 llm provider 注册表变化，热增/热减。 */
function syncVisionTwins(ctx, twinHandles) {
  let providers = [];
  try {
  ...
  if (/^https?:\/\//i.test(ref)) {
    ...
    if (name !== null && DRAFT_NAME_RE.test(name)) {
      ...
      try {
        ...
        throw new Error(`草稿图片不存在或已清理: ${name}（草稿保留 7 天）`);
      }
    }
...
/**
 * 输入框图片上传路由：接收 base64 图片，校验后落盘到 ~/.dsh/image-vision/drafts/，
...
function applyInner(ctx) {
  ...
  // 输入框图片协议：告诉模型「![图片](引用)」标记必须走 image_vision 工具识别。
  // 挂 ctx.effect：重载时自动注销旧 section，避免同名重复注册。
  ctx.effect(() => ctx.systemPrompt.section({
    ...
    text:
      ...
      "- 必须调用 image_vision 工具，把标记中的图片引用（本地路径或 i/文件名）作为 images 参数传入，逐张分析图片；\n" +
      "- 不要尝试用文件读取类工具直接读取该引用（二进制图片无法通过文本方式读取）；\n" +
      "- 分析完成后，把每张图片的内容整理成清晰的描述回复用户。",
  }));

  // 「+ 自动识图」twin 路由：为每个文本 provider 注册图片输入声明路由，
  // 让对话框原生粘贴/上传图片的完整流程（预览、发送、消息区渲染）可用。
  const twinHandles = new Map();
  ctx.effect(() => {
    syncVisionTwins(ctx, twinHandles);
    const off = ctx.on("llm/adapters-updated", () => syncVisionTwins(ctx, twinHandles));
    return () => {
      off();
  ...
  // ===== 像素级工具：裁剪 / 定位 / OCR（借鉴 dsh-vision-router 的多步迭代设计）=====
  // 工具输出的小图片（crop）会落盘到草稿目录并返回图片引用，模型可继续传给
  // image_vision / image_vision_ocr / image_vision_ground，形成「定位→裁剪→放大→再识别」闭环。

  /** 取当前配置的视觉模型（未配置时抛错）。 */
  function requireActiveVisionModel() {
    const config = normalizeConfig(configSource());
    const target = resolveTarget(config.providers, config.active);
    if (!target) {
      throw new Error("视觉模型未配置：请在 DSH 设置 → 视觉插件中添加供应商和模型，并选择使用");
    }
    return target;
  }
  ...
  async function askVisionModel(images, prompt, signal) {
    const target = requireActiveVisionModel();
    return callVisionModel(ctx, target.provider, target.model.id, images, prompt, signal);
  }
