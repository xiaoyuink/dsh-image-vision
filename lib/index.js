...
// 输入框粘贴/上传的图片草稿落盘目录（存放在插件安装目录下：~/.dsh/plugin/image-vision/drafts/）。
const DRAFTS_DIR = join(homedir(), ".dsh", "plugin", "image-vision", "drafts");
...
const SettingsSchema = z.object({
  enabled: z.boolean().default(false).description("总开关：开启后在模型选择器中显示视觉组（带小眼睛的模型）和输入框视觉模型选择器"),
...
/**
 * 输入框图片上传路由：接收 base64 图片，校验后落盘到 ~/.dsh/plugin/image-vision/drafts/，
...
function applyInner(ctx) {
  ...
  // 「+ 自动识图」twin 路由：仅当插件配置了视觉模型时才注册视觉组
  // （声明图片输入 → 原生图片附件的预览/发送/点开大图全流程可用；未配置时不显示）。
  const twinHandles = new Map();
  // 状态刷新：总开关开启时注册视觉组 + 识图工具 + 系统提示；关闭时全部注销（等效插件不存在）。
  function refreshState() {
    const on = configHasVisionModel(configSource());
    try { syncVisionTwins(ctx, twinHandles, on); } catch { /* 同步失败忽略 */ }
    if (on) {
        ...
        try {
          sysPromptDisposer = ctx.systemPrompt.section({
            ...
            text:
              ...
              "`![图片](i/iv-xxx.jpg)`（输入框粘贴/上传的草稿图片，文件名以 iv- 开头）或直接以「图片：」后跟图片引用。遇到这种标记时：\n" +
              "- **第一步：整图识别**。当用户询问「这是什么」或需要了解整张图片的内容时，直接调用 image_vision 工具，把标记中的图片引用（本地路径或 i/文件名）作为 images 参数传入，先用默认预设（general 整图描述）识别整张图片；不要在第一步就用 OCR 只读文字或拆分图片。\n" +
    ...
    defineTool({
      ...
      parameters: {
        images: {
          ...
          required: true,
          description:
            "图片引用列表，支持三种形式：① 本地绝对路径（C:/.../image.jpg）；" +
