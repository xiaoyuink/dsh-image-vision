...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var PLUGIN_VERSION = "2.7.2"; // 兜底常量；设置页实际显示以服务端 /config 返回的 version 为准（来自 package.json）
    ...
    function SettingsSection() {
      ...
      // 插件版本号：以服务端 /config 返回的 version 为准（来自 package.json 动态读取），读取前用兜底常量。
      var _ver = useState(PLUGIN_VERSION);
      var version = _ver[0]; var setVersion = _ver[1];
      // ===== 在线更新相关状态 =====
      ...
      var reload = async function () {
        try {
          ...
          if (typeof json.version === "string" && json.version !== "") setVersion(json.version);
        } catch (e) {
      ...
      return createElement("div", { className: "zGbnIq_section" },
        createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 } },
          ...
          createElement("span", { style: { display: "inline-flex", alignItems: "center", gap: 8, flexWrap: "wrap", justifyContent: "flex-end" } },
            createElement("span", { style: { fontSize: 12, color: "var(--dsw-alias-label-tertiary)", fontWeight: 400 } }, "v" + version),
