...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var PLUGIN_VERSION = "2.6.0"; // 与 package.json / host 侧 PLUGIN_VERSION 同步
    ...
    function installSendHook(conversation, isEnabled) {
      ...
      face.sendSession = async function (session, text, imageIds, mode) {
        ...
        if (vision) return original.call(face, session, text, imageIds, mode);
    ...
    function SettingsSection() {
      ...
      return createElement("div", { className: "zGbnIq_section" },
        ...
        createElement("div", { style: { display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", border: "1px solid var(--dsw-alias-border-l2)", borderRadius: 12, padding: "8px 14px" } },
          createElement("span", { style: { fontSize: 13, color: "var(--dsw-alias-label-secondary)" } }, "当前版本 v" + PLUGIN_VERSION),
          update !== null && update.updateAvailable
            ? createElement("span", { style: { fontSize: 13, color: "var(--dsw-alias-state-warn-label)" } }, "检测到新版本 v" + update.latestVersion)
            : null,
          update !== null && update.updateAvailable
            ? createElement("button", {
                type: "button",
          ...
          update !== null && update.updateAvailable
            ? createElement("a", {
                ...
                target: "_blank",
                rel: "noreferrer",
                style: { fontSize: 13, color: "var(--dsw-alias-link, var(--dsw-alias-primary))", textDecoration: "none" },
              }, "查看 Release")
            : null,
          updateMessage !== null ? createElement("span", { style: { fontSize: 13, color: "var(--dsw-alias-state-success-primary)" } }, updateMessage) : null
        ),
    ...
    function ImagePickButton(props) {
      ...
      useEffect(function () {
        var intercept = function (e) {
          ...
          e.preventDefault();
          e.stopPropagation();
          // 总开关关闭：不允许图片进入草稿（插件"完全退出"语义），明确提示后结束
          if (!enabledRef.current) {
            if (typeof window !== "undefined") {
              window.alert("插件已关闭：如需粘贴/拖拽/添加图片，请先在 设置 → 视觉插件 打开总开关。");
            }
            return;
          }
