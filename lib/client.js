...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * 结果直接显示、无滚动条：内容截断为两行内，悬停 title 看全文。
     */
    function testResultNode(testState) {
      ...
      if (result.ok) {
        ...
        var replyShort = reply.length > 150 ? reply.slice(0, 150) + "…" : reply;
        return createElement("div", { style: { fontSize: 12, lineHeight: 18, marginTop: 4, maxWidth: "100%" } },
      ...
      var errShort = errText.length > 150 ? errText.slice(0, 150) + "…" : errText;
      return createElement("div", { style: { fontSize: 12, lineHeight: 18, marginTop: 4, maxWidth: "100%" } },
        createElement("span", { style: { color: "var(--dsw-alias-state-error-primary)" } }, "检测失败：" + errShort),
    ...
    function SettingsSection() {
      ...
      return createElement("div", { className: "zGbnIq_section" },
        ...
        createElement("p", { className: "zGbnIq_intro" },
          ...
          "📌 在对话框直接粘贴/上传图片前，请先点击聊天页右下角模型选择器，选择带「+ 自动识图」的模型组（插件已自动注册），图片即可正常发送并在对话中显示。"),
    ...
    /**
     * 输入框工具行的「添加图片」按钮（conversation.input.left）。
     * - 点击按钮或直接粘贴/拖拽图片 → 上传落盘 → 在草稿中插入 `![图片](本地路径)` 标记；
     * - 发送后模型会按插件注入的系统规则调用 image_vision 工具识别图片。
     */
    /**
     * 输入框工具行「添加图片」按钮（conversation.input.left）。
     * 选图后模拟一次原生粘贴事件（DataTransfer + ClipboardEvent），
     * 图片进入 DSH 原生草稿（缩略图预览、发送、消息区渲染全走原生流程）。
     * 发送前请在模型选择器选择「+ 自动识图」模型组（插件注册的图片输入路由）。
     */
    function ImagePickButton(props) {
      ...
      return createElement("div", { className: "iv_pickWrap" },
        createElement(primitives.Tooltip, {
          label: "添加图片（也可直接粘贴/拖拽；发送前请选择「+ 自动识图」模型组）",
          side: "top",
          delayMs: 500,
        },
          createElement("button", {
