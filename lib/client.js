...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ===== 发送层 hook：把带图发送改写为附件存储引用（替代「+ 自动识图」视觉组准入）=====
    ...
    function ProviderEditor(props) {
      ...
      return createElement("li", { id: props.id, className: "zGbnIq_addCard iv_editingCard" },
        ...
        createElement(Field, { label: "API Key" },
          createElement(TextInput, { value: apiKey, type: "password", placeholder: hasExistingKey ? "已设置，留空保持不变（可填 env:变量名 引用）" : "sk-...（或 env:变量名，从环境变量读取）", onChange: setApiKey })
        ),
