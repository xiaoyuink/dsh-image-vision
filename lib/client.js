 ...
 * - 模型发现时，非识图模型禁用勾选并提示"该模型不具有识图能力，不可选择"。
 *
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * 模型检测结果渲染（ProviderCard / ProviderEditor 共用）。
     * testState: { busy: boolean, result: {ok, reply?, error?, imageSupport?, latencyMs?} } | null
     */
    function testResultNode(testState) {
      if (!testState) return null;
      if (testState.busy) {
        return createElement("div", { style: { fontSize: 12, lineHeight: 18, color: "var(--dsw-alias-label-tertiary)", marginTop: 4 } },
          "检测中…（用内置测试图实测，可能需要几秒）");
      }
      var result = testState.result;
      if (!result) return null;
      if (result.ok) {
        return createElement("div", { style: { fontSize: 12, lineHeight: 18, marginTop: 4 } },
          createElement("span", { style: { color: "var(--dsw-alias-state-success-primary)" } }, "检测成功（" + (result.latencyMs ?? "?") + "ms）："),
          createElement("div", { style: { color: "var(--dsw-alias-label-secondary)", marginTop: 2, wordBreak: "break-all" } }, String(result.reply ?? ""))
        );
      }
      return createElement("div", { style: { fontSize: 12, lineHeight: 18, marginTop: 4 } },
        createElement("span", { style: { color: "var(--dsw-alias-state-error-primary)" } }, "检测失败：" + String(result.error ?? "未知错误")),
        result.imageSupport === false
          ? createElement("div", { style: { color: "var(--dsw-alias-state-warn-label)", marginTop: 2 } },
            "该模型不支持图片输入，不具备识图能力")
          : null
      );
    }
    ...
    function ProviderCard(props) {
      ...
      for (var i = 0; i < p.models.length; i++) {
        (function (m) {
          ...
          modelRows.push(createElement("li", { className: "zGbnIq_modelEntry", key: m.id, style: isUsing ? { background: "var(--dsw-alias-interactive-bg-hover)" } : null },
            ...
            pickNodes.push(createElement("li", { className: "zGbnIq_candidate", key: dm.id },
              createElement("label", { className: "iv_candidateLabel", style: dm.vision ? null : { opacity: 0.55 } },
                createElement("input", {
                  ...
                  checked: checked,
                  disabled: !dm.vision,
                  title: dm.vision ? "" : "该模型不具有识图能力，不可选择",
      ...
      return createElement("li", { className: "zGbnIq_rowCard" + (isActiveProvider ? " iv_activeCard" : "") },
        createElement("div", { className: "zGbnIq_rowHead" },
    ...
    function ProviderEditor(props) {
      ...
      return createElement("li", { className: "zGbnIq_addCard" },
        createElement("div", { className: "zGbnIq_editorHeader" },
          createElement("span", { className: "zGbnIq_editorTitle" }, initial ? "编辑供应商" : "添加供应商"),
          initial
        ...
        createElement(Field, { label: "模型" },
          createElement("div", { className: "iv_inlineRow" },
            ...
            createElement("span", { style: { fontSize: 12, color: "var(--dsw-alias-label-tertiary)" } },
              apiKey.trim() === "" ? "未填 API Key，仅获取内置候选" : "非识图模型不可勾选")
          ),
      ...
      for (var i = 0; i < providers.length; i++) {
        (function (idx) {
          providerCards.push(createElement(ProviderCard, {
            ...
            onDelete: function () { removeProvider(providers[idx].id, providers[idx].name); },
          }));
        })(i);
      }

      return createElement("div", { className: "zGbnIq_section" },
        createElement("h2", { className: "zGbnIq_title" }, "识图插件"),
        createElement("p", { className: "zGbnIq_intro" },
          "插件会自动判断当前接入的模型是否识图：能识图则用当前模型并按预设提示词分析；" +
        ...
        createElement("ul", { className: "zGbnIq_rows" },
          ...
          editing !== null
            ? createElement(ProviderEditor, {
                key: "edit-" + providers[editing.index].id,
                initial: providers[editing.index],
                onSave: finishEdit,
        ...
        editing === null && !adding
          ? createElement("div", { className: "zGbnIq_addActions", style: { marginTop: 12 } },
              createElement("button", {
                type: "button",
